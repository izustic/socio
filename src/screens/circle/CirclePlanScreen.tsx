...
import LocationPickerModal, {
  type PickedEventLocation,
} from "@/src/components/circle/LocationPickerModal";
import {
  Colors,
  Radius,
  Spacing,
...
export default function CirclePlanScreen() {
  ...
  const [serverPublicLimitReached, setServerPublicLimitReached] =
    useState(false);
    ...
    const load = async () => {
      ...
      if (found) {
        ...
        setMembers(
          found.members
            .map((id) => byId.get(id))
            .filter(Boolean) as SwipeCandidate[],
        );
        setPlan({
          ...storedPlan,
  ...
  useEffect(() => {
    if (!user) return;
    void hasUsedMonthlyPublicEvent().then(setServerPublicLimitReached);
  }, [user]);
  ...
  const selectedExperience = plan.experiences.find(
    (event) => event.id === plan.selectedExperienceId,
  );
  const unanimousTime = Boolean(
    selectedTime &&
    circle &&
    selectedTime.votes.length >= circle.members.length,
  );
  const eventVoteComplete = Boolean(
    selectedExperience &&
    circle &&
    selectedExperience.votes.length >= circle.members.length,
  );
    ...
    return plan.experiences.filter((event) => {
      ...
      const matchesQuery =
        ...
        `${event.title} ${event.location} ${event.category}`
          ...
          .includes(needle);
      const matchesTime =
        !event.availableTimeIds ||
        !plan.selectedTimeId ||
        event.availableTimeIds.includes(plan.selectedTimeId);
      return matchesCategory && matchesQuery && matchesTime;
    });
  }, [category, plan.experiences, plan.selectedTimeId, query]);

  const publicEventLimitReached =
    !profile?.isSocioPlus && serverPublicLimitReached;

  useEffect(() => {
    if (plan.step !== "experience") return;
    let active = true;
    void getCommunityEvents(selectedTime?.startsAt)
      .then((events) => {
      ...
      .catch((error) =>
        console.warn("Could not load community events:", error),
      );
    return () => {
      active = false;
    };
  }, [plan.step, selectedTime?.startsAt, user?.id]);

  const voteForTime = (optionId: string) =>
    updatePlan((current) => ({
      ...current,
  ...
  const addCustomTime = (
    ...
    updatePlan((current) => ({
      ...
      timeOptions: [
        ...
        { id, label, detail, votes: [currentUserId], startsAt },
      ],
    }));
    setAddingTime(false);
  };

  const voteForExperience = (eventId: string) =>
    updatePlan((current) => ({
      ...current,
  ...
  const continueFromTime = () => {
    ...
    if (!unanimousTime) {
      Alert.alert(
        ...
        "Every Circle member must vote before experiences unlock.",
      );
      return;
    }
      ...
      <SafeAreaView style={styles.container}>
        <View style={styles.emptyState}>
          ...
          <Text style={styles.subtitle}>
            Join or create a Circle before planning a meetup.
          </Text>
    ...
    <SafeAreaView style={styles.container} edges={["top", "left", "right"]}>
          ...
          {[1, 2, 3, 4].map((step) => (
            <View
              ...
              style={[
                ...
                step <= stageNumber(plan.step) && styles.progressSegmentActive,
              ]}
            />
          ))}
        </View>
        ...
        <ExperienceStep
          ...
          onContinue={() => {
            ...
            if (!eventVoteComplete) {
              Alert.alert(
                "Waiting for everyone",
                "All Circle members need to vote for the experience before review.",
              );
              return;
            }
        ...
        <ReviewStep
          ...
          onConfirm={() =>
            updatePlan((current) => ({
              ...current,
              step: "scheduled",
              confirmedMemberIds: Array.from(
                new Set([...current.confirmedMemberIds, currentUserId]),
              ),
            }))
          }
        />
      ...
      {plan.step === "scheduled" && selectedTime && selectedExperience && (
        <ScheduledStep
          ...
          members={members}
          onBook={() =>
            selectedExperience.bookingUrl &&
            void Linking.openURL(selectedExperience.bookingUrl)
          }
        />
      )}
      ...
      <EventDetailModal
        event={detail}
        selectedTime={
          selectedTime ? `${selectedTime.label}, ${selectedTime.detail}` : ""
        }
        onClose={() => setDetail(null)}
        onVote={(id) => {
          voteForExperience(id);
          setDetail(null);
        }}
      />
      <CustomTimeModal
      ...
      <CreateEventModal
        ...
        selectedStartsAt={selectedTime?.startsAt}
        ...
        onCreate={(event) => {
          ...
          setCreating(false);
          if (event.isPublic) setServerPublicLimitReached(true);
        }}
      />
    </SafeAreaView>
  );
...
function TimeStep({
  ...
  const selected = plan.timeOptions.find(
    (option) => option.id === plan.selectedTimeId,
  );
  return (
    <View style={styles.flex}>
      ...
      >
        ...
        <Text style={styles.subtitle}>
          Pick one option. The next step unlocks when every member has voted.
        </Text>
          ...
          {plan.timeOptions.map((option) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  styles.optionCard,
                  selectedOption && styles.selectedCard,
                ]}
                onPress={() => onVote(option.id)}
              >
                <View style={styles.tintIcon}>
                  <CalendarDays size={21} color={Colors.primaryDark} />
                </View>
                <View style={styles.optionCopy}>
                  <Text style={styles.optionTitle}>{option.label}</Text>
                  <Text style={styles.optionMeta}>{option.detail}</Text>
                </View>
                <Text style={styles.voteCount}>
                  {option.votes.length} / {memberCount}
                </Text>
                <View style={[styles.radio, voted && styles.radioSelected]}>
                  {voted && <Check size={14} color={Colors.inverseText} />}
                </View>
              </TouchableOpacity>
            );
          })}
        </View>
        <TouchableOpacity style={styles.addOption} onPress={onSuggest}>
          <Plus size={18} color={Colors.textPrimary} />
          <Text style={styles.addOptionText}>Suggest another time</Text>
        </TouchableOpacity>
        <View style={styles.waitingCard}>
          ...
          <View style={styles.optionCopy}>
            <Text style={styles.optionTitle}>
              {selected
                ? `${selected.votes.length} of ${memberCount} voted`
                : "Choose your time"}
            </Text>
            <Text style={styles.optionMeta}>
              {selected && selected.votes.length < memberCount
                ? `Waiting for ${memberCount - selected.votes.length} member${memberCount - selected.votes.length === 1 ? "" : "s"}`
                : selected
                  ? "Everyone agrees — you can continue"
                  : "Your vote can be changed until everyone agrees"}
            </Text>
          </View>
          <View style={styles.avatarStack}>
            {members.slice(0, 3).map((member, index) => (
              <View
                key={member.uid}
                style={[styles.miniAvatar, { marginLeft: index ? -8 : 0 }]}
              >
                <Avatar
                  size={28}
...
function CustomTimeModal({
  ...
  const submit = () => {
    ...
    startsAt.setHours(numericHour, Number(minute), 0, 0);
    if (startsAt.getTime() <= Date.now()) {
      Alert.alert(
        "Choose a future time",
        "The event time needs to be later than now.",
      );
      return;
    }
    onCreate(
      ...
      <SafeAreaView style={styles.modalPage}>
        <View style={styles.modalHeader}>
          ...
          <View style={styles.headerCopy}>
            ...
            <Text style={styles.headerSubtitle}>
              Everyone will vote on this option
            </Text>
          </View>
        ...
        <ScrollView contentContainerStyle={styles.modalContent}>
          <View style={styles.customTimeHero}>
            ...
            <Text style={styles.optionMeta}>
              Suggestions must be within the next 30 days.
            </Text>
          </View>
            ...
            <View style={styles.calendarHeader}>
              ...
              >
                <ChevronRight
                  size={20}
                  color={
                    canGoForward ? Colors.textPrimary : Colors.textDisabled
                  }
                />
              </TouchableOpacity>
            </View>
...
function ExperienceStep(props: {
  ...
  memberCount: number;
  selectedTime?: { label: string; detail: string };
  onCategory: (value: MeetupCategory) => void;
  onQuery: (value: string) => void;
  onCreate: () => void;
  onDetail: (event: MeetupExperience) => void;
  onVote: (id: string) => void;
  onContinue: () => void;
}) {
  return (
    <View style={styles.flex}>
      ...
      >
        <View style={styles.sectionTitleRow}>
          <View style={styles.optionCopy}>
            <Text style={styles.eyebrow}>STEP 2 · EXPERIENCE</Text>
            <Text style={styles.title}>Pick a place</Text>
          </View>
          <TouchableOpacity style={styles.createLink} onPress={props.onCreate}>
            <Plus size={16} color={Colors.primaryDark} />
            <Text style={styles.createLinkText}>Create event</Text>
          </TouchableOpacity>
        </View>
        <Text style={styles.subtitle}>
          Showing experiences available{" "}
          {props.selectedTime
            ? `${props.selectedTime.label} at ${props.selectedTime.detail}`
            : "at your chosen time"}
          .
        </Text>
          ...
          {CATEGORIES.map((item) => {
            ...
            return (
              <TouchableOpacity
                ...
                onPress={() => props.onCategory(item)}
              >
                <Icon
                  size={16}
                  color={active ? Colors.inverseText : Colors.textSecondary}
                />
                <Text
                  style={[
                    styles.categoryText,
        ...
        <View style={styles.recommendationRow}>
          <Text style={styles.listHeading}>Recommended for your Circle</Text>
          <Text style={styles.timeFit}>Time matched</Text>
        </View>
        {props.experiences.map((event) => (
          <ExperienceCard
            key={event.id}
            event={event}
            memberCount={props.memberCount}
            onPress={() => props.onDetail(event)}
            onVote={() => props.onVote(event.id)}
          />
        ))}
        {!props.experiences.length && (
          <View style={styles.noResults}>
            <Search size={28} color={Colors.textDisabled} />
            <Text style={styles.optionTitle}>No matching events</Text>
            <Text style={styles.optionMeta}>
      ...
      <View style={styles.footer}>
        <Button title="Review meetup" onPress={props.onContinue} />
      </View>
    </View>
  );
}
      ...
      <View style={styles.eventArtwork}>
        ...
        {event.sponsored && (
          <View style={styles.sponsoredBadge}>
            <Star size={10} color={Colors.primaryDark} />
      ...
      <View style={styles.eventBody}>
        ...
        <View style={styles.eventFooter}>
          <Text style={styles.fitBadge}>
            {event.isCreatedByCircle ? "Your event" : "Good for new Circles"}
          </Text>
          <TouchableOpacity
            style={styles.smallVote}
            onPress={(e) => {
              e.stopPropagation();
              onVote();
            }}
          >
            <Text style={styles.smallVoteText}>
              {event.votes.length
                ? `${event.votes.length}/${memberCount} voted`
                : "Vote"}
            </Text>
          </TouchableOpacity>
        </View>
  ...
  return (
    <View style={styles.flex}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <Text style={styles.eyebrow}>STEP 3 · REVIEW</Text>
        <Text style={styles.title}>Review & confirm</Text>
        <Text style={styles.subtitle}>
          Double-check the plan before we notify everyone.
        </Text>
        <View style={styles.reviewHero}>
          ...
          <Text style={styles.reviewSubtitle}>
            {experience.category} · {experience.price}
          </Text>
        </View>
        <View style={styles.detailCard}>
          <DetailRow icon={Clock3} label="When" value={time} />
          <DetailRow icon={MapPin} label="Where" value={experience.location} />
          <DetailRow
            icon={Ticket}
            label="Booking"
            value={
              experience.bookingUrl
                ? "Required · booking link available"
                : "No booking required"
            }
          />
          <DetailRow
        ...
        <Text style={styles.listHeading}>Circle confirmations</Text>
        <View style={styles.confirmationRow}>
          {members.map((member) => {
            const ready =
              confirmed.includes(member.uid) || member.uid === currentUserId;
            return (
              <View key={member.uid} style={styles.confirmationMember}>
                <View>
                  <Avatar
                    size={46}
                    uri={member.photoURL || undefined}
                  ...
                  {ready && (
                    <View style={styles.readyDot}>
                      <Check size={10} color={Colors.inverseText} />
                    </View>
                  )}
                </View>
                <Text numberOfLines={1} style={styles.confirmationName}>
        ...
        {experience.bookingUrl && (
          <View style={styles.bookingNotice}>
            <Ticket size={21} color={Colors.primaryDark} />
            <View style={styles.optionCopy}>
              <Text style={styles.optionTitle}>Booking required</Text>
              <Text style={styles.optionMeta}>
                After confirmation, members can open the organizer booking page.
              </Text>
            </View>
          </View>
        )}
      ...
      <View style={styles.footer}>
        <Button title="Confirm meetup" onPress={onConfirm} />
        <Button title="Change experience" variant="ghost" onPress={onBack} />
      </View>
    </View>
  );
}
...
function ScheduledStep({
  ...
  return (
    <ScrollView contentContainerStyle={styles.scheduled}>
      <View style={styles.successIcon}>
        <CalendarDays size={42} color={Colors.success} />
        <View style={styles.successCheck}>
          <Check size={14} color={Colors.inverseText} />
        </View>
      </View>
      <Text style={styles.scheduledTitle}>Meetup scheduled!</Text>
      <Text style={styles.scheduledSubtitle}>
        Your Circle has a plan. We’ll keep the details here for everyone.
      </Text>
      ...
      {experience.bookingUrl && (
        <Button title="Open booking link" onPress={onBook} />
      )}
      <Button
        title="Go to Circle chat"
        variant={experience.bookingUrl ? "outline" : "primary"}
        ...
        { text: tx("common.cancel"), style: "cancel" },
        {
          ...
          onPress: () =>
            void reportCommunityEvent(
              event.id,
              "Inappropriate or misleading listing",
            )
              .then(() =>
                Alert.alert(
      ...
      <SafeAreaView style={styles.modalPage}>
        ...
        <ScrollView contentContainerStyle={styles.modalContent}>
          <View style={styles.detailArtwork}>
            <Icon size={54} color={Colors.primaryDark} />
            {event.sponsored && (
              <View style={styles.sponsoredLarge}>
                <Star size={13} color={Colors.primaryDark} />
                <Text style={styles.sponsoredText}>Sponsored listing</Text>
              </View>
            )}
          </View>
          {event.isCommunityEvent && (
...
function CreateEventModal({
  ...
  publicEventLimitReached: boolean;
  onClose: () => void;
  onCreate: (event: MeetupExperience) => void;
}) {
  const [name, setName] = useState("");
  const [location, setLocation] = useState<PickedEventLocation | null>(null);
  ...
  const [category, setCategory] =
    useState<Exclude<MeetupCategory, "All">>("Food");
  const [media, setMedia] = useState<PendingEventMedia[]>([]);
  ...
  const pickMedia = async () => {
    ...
    const tooLong = result.assets.some(
  ...
  const submit = async () => {
    ...
    try {
      ...
      const baseEvent: MeetupExperience = {
        id: eventId,
        title: name.trim(),
        ...
        expiresAt: isPublic ? selectedStartsAt : undefined,
        startsAt: selectedStartsAt,
        availableTimeIds: selectedTimeId ? [selectedTimeId] : undefined,
        votes: [],
      };
      const event = isPublic
        ? await publishCommunityEvent({
            ...
            startsAt: selectedStartsAt!,
            bookingUrl: baseEvent.bookingUrl,
            priceLabel: baseEvent.price,
            capacity: parsedCapacity,
            media: uploadedMedia,
          })
        : baseEvent;
        ...
        <ScrollView contentContainerStyle={styles.modalContent}>
          ...
          >
            ...
            {media.map((item) => (
              <View key={item.id} style={styles.mediaPreview}>
                {item.type === "image" ? (
                  ...
                  <View style={styles.videoPreview}>
                    ...
                    <Text style={styles.videoDuration}>
                      {Math.ceil((item.durationMs ?? 0) / 1000)}s
                    </Text>
                  </View>
                )}
                <TouchableOpacity
                ...
                >
                  <X size={14} color={Colors.inverseText} />
                </TouchableOpacity>
              </View>
            ))}
          </ScrollView>
          ...
          >
            {CATEGORIES.slice(1).map((item) => (
              <TouchableOpacity
                ...
                onPress={() =>
                  setCategory(item as Exclude<MeetupCategory, "All">)
                }
              >
                <Text
                  style={[
          ...
          <TouchableOpacity
            style={[
              ...
              location && styles.locationPickerButtonSelected,
            ]}
          ...
          <View style={styles.visibilityRow}>
            <TouchableOpacity
              style={[
                ...
                !isPublic && styles.visibilityChoiceActive,
              ]}
...
const styles = createThemedStyles((Colors) => ({
  ...
  headerSubtitle: { ...Typography.bodySmall, fontSize: 11 },
  stepBadge: {
  ...
  scrollContent: { padding: Spacing.lg, paddingBottom: 36 },
  eyebrow: { ...Typography.label, color: Colors.primaryDark, marginBottom: 7 },
  title: { ...Typography.h2, marginBottom: 7 },
  ...
  optionMeta: { ...Typography.bodySmall, fontSize: 12 },
  voteCount: { ...Typography.label, color: Colors.textPrimary },
  radio: {
    width: 23,
    height: 23,
    borderRadius: 12,
  ...
  createLinkText: { ...Typography.label, color: Colors.primaryDark },
  searchBar: {
    flexDirection: "row",
    alignItems: "center",
    gap: 9,
  ...
  categories: { gap: 8, paddingBottom: 16 },
  categoryChip: {
    flexDirection: "row",
    alignItems: "center",
    gap: 5,
  ...
  categoryTextActive: { color: Colors.inverseText },
  recommendationRow: {
    flexDirection: "row",
    alignItems: "center",
  ...
  timeFit: { ...Typography.label, color: Colors.success, fontSize: 10 },
  eventCard: {
    flexDirection: "row",
    padding: 10,
    marginBottom: 10,
  ...
  eventBody: { flex: 1, paddingLeft: 12 },
  eventTitleRow: { flexDirection: "row", alignItems: "center" },
  eventTitle: { ...Typography.body, fontWeight: "700", flex: 1 },
  eventMeta: { ...Typography.bodySmall, fontSize: 11, marginTop: 2 },
  eventPrice: { ...Typography.label, color: Colors.textPrimary, marginTop: 5 },
  eventFooter: {
    marginTop: "auto",
    flexDirection: "row",
    alignItems: "center",
  ...
  noResults: { alignItems: "center", gap: 7, paddingVertical: 40 },
  monetizationNote: {
    flexDirection: "row",
    gap: 10,
    borderRadius: Radius.md,
  ...
  reviewTitle: { ...Typography.h3, textAlign: "center" },
  reviewSubtitle: { ...Typography.bodySmall, marginTop: 4 },
  detailCard: {
    borderWidth: 1,
    borderColor: Colors.border,
    borderRadius: Radius.md,
  ...
  detailIcon: { width: 31 },
  detailLabel: { ...Typography.label, width: 64 },
  detailValue: {
    ...Typography.bodySmall,
    color: Colors.textPrimary,
    flex: 1,
  ...
  confirmationMember: { width: 54, alignItems: "center", gap: 5 },
  confirmationName: { ...Typography.label, fontSize: 9, maxWidth: 54 },
  readyDot: {
    position: "absolute",
    right: -2,
    bottom: -2,
  ...
  scheduledTitle: { ...Typography.h1, fontSize: 29, textAlign: "center" },
  scheduledSubtitle: {
    ...Typography.bodySmall,
    textAlign: "center",
    marginBottom: 10,
  ...
  expiryText: { ...Typography.bodySmall, textAlign: "center", fontSize: 11 },
  ...
  linkText: { ...Typography.label, color: Colors.primaryDark },
  ...
  formValue: { ...Typography.body, fontWeight: "600" },
  visibilityRow: { flexDirection: "row", gap: 10, marginBottom: 12 },
  visibilityChoice: {
    flex: 1,
    gap: 3,
    borderWidth: 1,
  ...
  reportLinkText: { ...Typography.label, color: Colors.danger },
}));
