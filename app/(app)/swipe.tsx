  ...
  const handleSwipe = async (liked: boolean) => {
    ...
    try {
      ...
      if (result.circleComplete) {
        Alert.alert('Circle complete!', 'Your circle is now full. Opening dashboard.');
        router.replace({
          pathname: '/(app)/circle-dashboard',
  ...
  if (!currentCandidate) {
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.centerContent}>
          <View style={styles.emptyArtwork} />
          <Text style={styles.emptyTitle}>No people to swipe right now</Text>
          <Text style={styles.emptySubtitle}>
            We will keep looking. Check back soon for new matches in your area.
          </Text>
          <View style={styles.emptyCtaWrap}>
            <Button title="Refresh" onPress={loadSwipeData} />
          </View>
        </View>
      </SafeAreaView>
    );
