-- Host swipes users; uses same uuid / circle_status handling as joiner RPC.
...
as $$
declare
  v_host_uuid uuid := auth.uid();
  v_host_text text := auth.uid()::text;
  v_target_uuid uuid := p_target_user_id::uuid;
...
begin
  ...
  if v_row.creator_id is distinct from v_host_uuid then
  ...
  if p_liked then
    ...
    if v_mutual and coalesce(array_length(v_members, 1), 0) < v_size then
      if not v_target_uuid = any (v_members) then
        v_members := array_append(v_members, v_target_uuid);
        v_added := true;
      else
        v_added := true;
      end if;
