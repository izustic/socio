...
const mapUserToDbUpdates = (updates: Partial<User>): Record<string, unknown> => {
  ...
  if (updates.isSocioPlus !== undefined) dbUpdates.is_socio_plus = updates.isSocioPlus;
  if (updates.subscriptionStatus !== undefined) dbUpdates.subscription_status = updates.subscriptionStatus;
  if (updates.subscriptionPlatform !== undefined) dbUpdates.subscription_platform = updates.subscriptionPlatform;
  if (updates.subscriptionProductId !== undefined) dbUpdates.subscription_product_id = updates.subscriptionProductId;
  if (updates.subscriptionExpiresAt !== undefined) {
    dbUpdates.subscription_expires_at =
      updates.subscriptionExpiresAt instanceof Date
  ...
  try {
    ...
    if (data) {
      return {
        ...
        isSocioPlus: data.is_socio_plus ?? false,
        subscriptionStatus: data.subscription_status ?? "inactive",
        subscriptionPlatform: data.subscription_platform ?? null,
