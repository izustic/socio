...
begin
  ...
  if v_row.creator_id = v_user_id then
    raise exception 'Hosts must swipe users from the host flow';
  end if;

  if v_row.status <> 'forming' then
  ...
  if p_liked then
    v_user_pending := coalesce(v_pending -> v_user_id, '[]'::jsonb);
    if not v_user_pending @> to_jsonb(p_circle_id) then
      v_user_pending := v_user_pending || to_jsonb(p_circle_id);
    end if;
    v_pending := jsonb_set(v_pending, array[v_user_id], v_user_pending, true);

    v_host_pending := coalesce(v_pending -> v_row.creator_id, '[]'::jsonb);
    v_mutual := v_host_pending @> to_jsonb(v_user_id);
    ...
    v_user_pending := coalesce(v_skipped -> v_user_id, '[]'::jsonb);
    if not v_user_pending @> to_jsonb(p_circle_id) then
      v_user_pending := v_user_pending || to_jsonb(p_circle_id);
    end if;
    v_skipped := jsonb_set(v_skipped, array[v_user_id], v_user_pending, true);
  end if;
  ...
  set
    ...
    members = v_members,
    status = case when v_complete then 'complete' else 'forming' end
  where id = p_circle_id;
