...
export type UploadStage = "picking" | "compressing" | "uploading" | "complete";
...
const fetchBlob = async (uri: string, contentType: string = "image/jpeg") => {
...
const validateVideo = async (asset: ImagePicker.ImagePickerAsset) => {
...
export const uploadProfileMedia = async ({
  ...
  try {
    ...
    const blob = await fetchBlob(processedUri, contentType);
